...
if Qulight["datatext"].Gold and Qulight["datatext"].Gold > 0 then
	...
	local function OnEvent(self, event)
		...
		self:SetScript("OnEnter", function()
				...
				GameTooltip:AddLine("事件信息: ")
				GameTooltip:AddDoubleLine("获得:", formatMoney(Profit), 1, 1, 1, 1, 1, 1)
				GameTooltip:AddDoubleLine("支出:", formatMoney(Spent), 1, 1, 1, 1, 1, 1)
				if Profit < Spent then
					GameTooltip:AddDoubleLine("损失:", formatMoney(Profit-Spent), 1, 0, 0, 1, 1, 1)
				elseif (Profit-Spent)>0 then
					GameTooltip:AddDoubleLine("收入:", formatMoney(Profit-Spent), 0, 1, 0, 1, 1, 1)
				end
				GameTooltip:AddLine' '
				...
				GameTooltip:AddDoubleLine("总额: ", FormatTooltipMoney(totalGold), 1, 1, 1, 1, 1, 1)
